...
impl TtlVaultContract {
    ...
    fn trigger_release_internal(env: Env, vault_id: u64, mode: ReleaseTrigger) {
        ...
        if has_vesting || has_milestone_vesting {
            ...
            if vault.beneficiaries.is_empty() {
                // Transfer net amount to primary beneficiary
                token_client.transfer(&env.current_contract_address(), &vault.beneficiary, &net_amount);
                env.events().publish(
                    (RELEASE_TOPIC,),
                    ReleaseEvent { vault_id, beneficiary: vault.beneficiary.clone(), amount: net_amount },
                );
            } else {
