...
fn find_event_by_topic<T>(
    env: &Env,
    target_topic: soroban_sdk::Symbol,
) -> Option<T>
where
...
fn test_create_vault_emits_vault_created_event() {
    ...
    assert_eq!(event.check_in_interval, interval, "check_in_interval mismatch");
}
...
fn test_create_vault_event_vault_id_increments() {
    ...
    let vault_events: alloc::vec::Vec<VaultCreatedEvent> = env.events().all()
        .iter()
        .filter_map(|e| {
            let topics: soroban_sdk::Vec<soroban_sdk::Val> = e.1.clone().into_val(&env);
            let topic0: Result<soroban_sdk::Symbol, _> =
...
fn test_check_in_event_new_ttl_advances_with_each_check_in() {
    ...
    assert!(e2.new_ttl > e1.new_ttl, "new_ttl should advance after each check-in");
}
...
fn test_trigger_release_event_amount_matches_balance() {
    ...
    let event: ReleaseEvent = find_event_by_topic(&env, types::RELEASE_TOPIC)
        .expect("ReleaseEvent not emitted");
...
fn test_update_beneficiary_emits_beneficiary_updated_event() {
    ...
    assert_eq!(event.vault_id, vault_id, "vault_id mismatch");
    assert_eq!(event.old_beneficiary, old_beneficiary, "old_beneficiary mismatch");
    assert_eq!(event.new_beneficiary, new_beneficiary, "new_beneficiary mismatch");
}
...
fn test_update_beneficiary_event_topic_contains_vault_id() {
    ...
    let found = env.events().all().iter().any(|e| {
        ...
        topic0.map(|s| s == types::BENEFICIARY_UPDATED_TOPIC).unwrap_or(false)
            && topic1.map(|id| id == vault_id).unwrap_or(false)
    });

    assert!(found, "BENEFICIARY_UPDATED_TOPIC topic does not contain vault_id");
}
